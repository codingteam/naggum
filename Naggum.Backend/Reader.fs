...
let numberOptions =
    NumberLiteralOptions.AllowMinusSign ||| NumberLiteralOptions.AllowExponent ||| NumberLiteralOptions.AllowHexadecimal
    ||| NumberLiteralOptions.AllowFraction ||| NumberLiteralOptions.AllowSuffix
    ...
    fun stream ->
        ...
        if reply.Status = Ok then
            ...
            if result.IsInteger then
                ...
                else if not (result.SuffixLength = 1) then Reply((int32 result.String) :> obj |> Object)
                else Reply(ReplyStatus.Error, messageError <| sprintf "Unknown suffix: %A" result.SuffixChar1)
            else if result.SuffixLength = 1 && result.SuffixChar1 = 'f' then
                Reply((float result.String) :> obj |> Object)
            else if not (result.SuffixLength = 1) then Reply((single result.String) :> obj |> Object)
            else Reply(ReplyStatus.Error, messageError <| sprintf "Unknown suffix: %A" result.SuffixChar1)
